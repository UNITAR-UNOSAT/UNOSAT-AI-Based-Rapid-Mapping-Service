...
def model(input_shape=(64,64,3), classes=3, kernel_size = 3, filter_depth = (12,24,48,96,192)):
    ...
    #37x37
    x = Conv2D(filter_depth[4], (kernel_size, kernel_size), padding="same")(pool4)
    x = Activation("relu")(x)
    x = Conv2D(filter_depth[4], (kernel_size, kernel_size), padding="same")(x)
    x = Activation("relu")(x)
    ...
    conv9 = Conv2D(filter_depth[3], (kernel_size, kernel_size), padding="same")(up1)
    act9 = Activation("relu")(conv9)
    conv10 = Conv2D(filter_depth[3], (kernel_size, kernel_size), padding="same")(act9)
    act10 = Activation("relu")(conv10)
    concat1 = Concatenate()([act8, act10])
    
    up2 = UpSampling2D(size=(2, 2))(concat1)
    conv11 = Conv2D(filter_depth[2], (kernel_size, kernel_size), padding="same")(up2)
    act11 = Activation("relu")(conv11)
    conv12 = Conv2D(filter_depth[2], (kernel_size, kernel_size), padding="same")(act11)
    act12 = Activation("relu")(conv12)
    concat2 = Concatenate()([act6,act12])
    # 74x74
    
    up3 = UpSampling2D(size=(2, 2))(concat2)
    conv13 = Conv2D(filter_depth[1], (kernel_size, kernel_size), padding="same")(up3)
    act13 = Activation("relu")(conv13)
    conv14 = Conv2D(filter_depth[1], (kernel_size, kernel_size), padding="same")(act13)
    act14 = Activation("relu")(conv14)
    concat3 = Concatenate()([act4,act14])
    # 148x148
    
    up4 = UpSampling2D(size=(2, 2))(concat3)
    conv15 = Conv2D(filter_depth[0], (kernel_size, kernel_size), padding="same")(up4)
    act15 = Activation("relu")(conv15)
    conv16 = Conv2D(filter_depth[0], (kernel_size, kernel_size), padding="same")(act15)
    act16 = Activation("relu")(conv16)
    concat4 = Concatenate()([act2,act16])
    
    conv17 = Conv2D(classes, (1,1), padding="valid")(concat4)
    
    reshape1 = Reshape((input_shape[0]*input_shape[1],classes))(conv17)
    act17 = Activation("softmax")(reshape1)
    
    model = Model(img_input, act17)
